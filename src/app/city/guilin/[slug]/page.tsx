...
function findContent(slug: string): { meta: Record<string, unknown>; content: string; category: string } | null {
  ...
  for (const dir of dirs) {
    const filePath = dir
      ? `cities/guilin/${dir}/${slug}.mdx`
      : `cities/guilin/${slug}.mdx`
    try {
...
export function generateStaticParams() {
  const attractions = getContentSlugs('cities/guilin/attractions')
  const food = getContentSlugs('cities/guilin/food')
  const base = getContentSlugs('cities/guilin').filter(s => s !== 'index')
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://chinabound.online/city/guilin/${slug}/`,
    },
  }
}

export default async function GuilinContentPage({ params }: Props) {
  const { slug } = await params
  const data = findContent(slug)
  if (!data) notFound()

  const { meta, content } = data
  const pageUrl = `https://chinabound.online/city/guilin/${slug}/`
    ...
    <div className="page-container-narrow">
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: 'Guilin', url: 'https://chinabound.online/city/guilin/' },
      ...
      <nav className="breadcrumb">
        ...
        <Link href="/city/guilin/">Guilin</Link>
        <span className="breadcrumb-sep">/</span>
      ...
      <div style={{ marginTop: '3rem', paddingTop: '2rem', borderTop: '1px solid var(--border)' }}>
        <Link href="/city/guilin/" className="link-accent">
          ← Back to Guilin Guide
        </Link>
      </div>
    </div>
  )
