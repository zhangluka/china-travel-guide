...
function findContent(slug: string): { meta: Record<string, unknown>; content: string; category: string } | null {
  ...
  for (const dir of dirs) {
    const filePath = dir
      ? `cities/chongqing/${dir}/${slug}.mdx`
      : `cities/chongqing/${slug}.mdx`
    try {
...
export function generateStaticParams() {
  const attractions = getContentSlugs('cities/chongqing/attractions')
  const food = getContentSlugs('cities/chongqing/food')
  const base = getContentSlugs('cities/chongqing').filter(s => s !== 'index')
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://chinabound.online/city/chongqing/${slug}/`,
    },
  }
}

export default async function ChongqingContentPage({ params }: Props) {
  const { slug } = await params
  const data = findContent(slug)
  if (!data) notFound()
  ...
  const pageUrl = `https://chinabound.online/city/chongqing/${slug}/`
    ...
    <div className="page-container-narrow">
      ...
      <BreadcrumbJsonLd
        items={[
          ...
          { name: 'Chongqing', url: 'https://chinabound.online/city/chongqing/' },
      ...
      <nav className="breadcrumb">
        ...
        <Link href="/city/chongqing/">Chongqing</Link>
        <span className="breadcrumb-sep">/</span>
      ...
      <div style={{ marginTop: '3rem', paddingTop: '2rem', borderTop: '1px solid var(--border)' }}>
        <Link href="/city/chongqing/" className="link-accent">
